...
then
    ...
    sedInPlace "s%COPY --from=builder /choco-solver/parsers/target/choco-parsers-*%COPY --from=builder /choco-solver/parsers/target/choco-parsers-$VERSION-jar-with-dependencies.jar /entry_data/choco-parsers.jar%"  ./parsers/src/main/docker/Dockerfile_Choco.dms
