...
public class SparseBitSet implements IStateBitSet {
     ...
     * @implSpec In that case, block size is set to 64.
     */
    public SparseBitSet(final IEnvironment env, final BitSet initial) {
        this.env = env;
        this.blockSize = 64;
        long[] words = initial.toLongArray();
        index = new S64BitSet(env);
        this.blocks = new IStateBitSet[words.length];
        for (int i = 0; i < words.length; i++) {
            if (words[i] != 0) {
                this.index.set(i);
                this.blocks[i] = new OneWordS64BitSet(env, words[i]);
            }
        }
    }
  ...
  private IStateBitSet ensureBlock(final int blockIndex) {
    if (blocks[blockIndex] == null) {
      ...
      blocks[blockIndex] = new S64BitSet(env, blockSize);
    }
