 ...
 * Time: 16:05
 */
public class DynamicSweepMinKDim {
    ...
    // h[task][resource]
	public DynamicSweepMinKDim(CumulativeKDim rsc, Propagator prop, int n, int[] limit, int[] ls, int[] us, int[] ld, int[] le, int[] ue, int[][] h) {
	...
	protected void generateMinEvents() {
		...
		for(int t=0;t<n;t++) {
			...
			if (cp[t]) {
				...
				hEvents.add(us[t],t,Event.CCP,-1);
			}
		}
	}
...
//System.out.println("task "+t+" removed from hConflict["+r+"]");
                if ( delta >= us[t] ) {
                    ...
                    if ( date - delta >= ld[t] && rc == -1 ) { // no conflict; sweep interval large enough
                        adjust = true; mins[t] = delta;
                     } else if ( date - delta < ld[t] && rc == -1 ) { // no conflict; sweep interval too small
                        state[t] = CHECK;
                        mins[t] = delta;
        ...
        for (int r=0;r<rsc.nbResources();r++) {
            while ( !hCheck[r].isEmpty() && (hEvents.isEmpty() || hCheck[r].peekHeight() > gap[r]) ) {
                ...
                if ( delta >= us[t] || hEvents.isEmpty() || (delta-mins[t] >= ld[t] && state[t] == CHECK) ) {
                    pruning |= adjustTask(t);
