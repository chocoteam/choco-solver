...
public class DynamicSweepMin {
	...
	public DynamicSweepMin(int nbTasks, Propagator prop,int n, int limit, int[] ls, int[] us, int[] ld, int[] le, int[] ue, int[] h) {
		//assert(n == ls.length && n == us.length && n == ld.length && n == le.length && n == ue.length && n == h.length);
		this.prop = prop;
		this.n = n;
		this.capa = limit;
		this.hEvents = new IncHeapEvents(4*n+2*nbTasks);
