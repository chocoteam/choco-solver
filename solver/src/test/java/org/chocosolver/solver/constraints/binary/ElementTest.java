...
public class ElementTest {
    ...
    public void testElt2D() {
        Model m = new Model();
        IntVar value = m.intVar("value", 0, 5);
        int[][] values = {{0, 1, 2}, {3, 4, 5}};
        IntVar[] index = {
                m.intVar("index1", 0, 1),
                m.intVar("index2", 0, 2)
        };
        m.element(value, values, index[0], 0, index[1], 0);
        while (m.getSolver().solve()) {
//            System.out.println(value);
//            System.out.println(index[0]);
//            System.out.println(index[1]);
//            System.out.println(values[index[0].getValue()][index[1].getValue()]);
        }
        Assert.assertEquals(m.getSolver().getSolutionCount(), 6);
    }

    @Test(groups = "1s")
    public void testElt2D2() {
        for (int seed = 0; seed < 200; seed++) {
            Model m = new Model();
            IntVar value = m.intVar("value", 0, 17);
            IntVar[][] values = new IntVar[2][3];
            int k = 0;
            for (int i = 0; i < 2; i++) {
                for (int j = 0; j < 3; j++) {
                    values[i][j] = m.intVar("v_" + i + "_" + j, k, k + 1);
                    k += 3;
                }
            }
            IntVar[] index = {
            ...
            m.element(value, values, index[0], 0, index[1], 0);
            m.getSolver().setSearch(new FullyRandom(m.retrieveIntVars(true), seed));
            while (m.getSolver().solve()) {
//                System.out.println(value);
//                System.out.println(index[0]);
//                System.out.println(index[1]);
...
//                System.out.println("---");
            ...
            Assert.assertEquals(m.getSolver().getSolutionCount(), 384);
        }
    }
}